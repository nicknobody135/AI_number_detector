...
int training_set_num = 1;
char file_locations_of_training_data[30][35] = {"training_data\\set_a\\0s(a).txt" , "training_data\\set_a\\1s(a).txt" , "training_data\\set_a\\2s(a).txt" , "training_data\\set_a\\3s(a).txt" , "training_data\\set_a\\4s(a).txt" , "training_data\\set_a\\5s(a).txt" , "training_data\\set_a\\6s(a).txt" , "training_data\\set_a\\7s(a).txt" , "training_data\\set_a\\8s(a).txt" , "training_data\\set_a\\9s(a).txt" , "training_data\\set_b\\0s(b).txt" , "training_data\\set_b\\1s(b).txt" , "training_data\\set_b\\2s(b).txt" , "training_data\\set_b\\3s(b).txt" , "training_data\\set_b\\4s(b).txt" , "training_data\\set_b\\5s(b).txt" , "training_data\\set_b\\6s(b).txt" , "training_data\\set_b\\7s(b).txt" , "training_data\\set_b\\8s(b).txt" , "training_data\\set_b\\9s(b).txt" , "training_data\\set_a\\example.txt"};
FILE *target_file;
...
void initialize_weights_and_biases()
{
    weights_and_biases = fopen("ai_stuff\\new_neural_network_weights_and_biases.bin" , "rb");
...
void forward_propagation(bool display_results_or_not)
{
    ...
    for(int a = 0 ; a < 6 ; a++)
    {
        ...
        if(a == 5)
        {
            ...
            {
                if(forward_prop_results[c] > max)max = forward_prop_results[c];
            }
            float sum = 0.0f;
            for(int c = 3584 ; c < 3594 ; c++)
            {
